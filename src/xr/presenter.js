...
export function createPresenter(canvas) {
  ...
  /**
   * Query. Check the legacy bridge every frame, reporting the exact failing stage.
   * @param {string} stage - The operation just completed.
   * @example check('scene upload') // undefined when the GL error queue is empty
   */
  function check(stage) {
    const error = gl.getError();
    if (error !== gl.NO_ERROR) throw new Error(`WebXR WebGL ${stage} failed: ${error} (0x${error.toString(16)}). Native WebGPU XR requires visionOS 26.2+.`);
  }
  ...
  function upload(source, index) {
    ...
    check(index === 0 ? 'WebGPU canvas upload' : 'control panel upload');
  }
