...
export class PhysicsWorld {
    ...
    /**
     * Command. Applies a jump impulse if a downward ray hits ground, excluding the player.
     * @example physics.jump() // undefined; grounded player receives upward impulse
     */
    jump() {
        const t = this.playerBody.translation();
        ...
        const hit = this.world.castRay(
            ray, PLAYER_HALF_HEIGHT + PLAYER_RADIUS + 0.15, true,
            undefined, undefined, undefined, this.playerBody,
        );
        if (hit) {
    ...
    shoot(pos, dirX, dirY, dirZ, offset = 1.5) {
        ...
        // Spawn at the caller's muzzle offset (desktop default clears the player capsule).
        const rbDesc = RAPIER.RigidBodyDesc.dynamic()
            .setTranslation(pos.x + dirX * offset, pos.y + dirY * offset, pos.z + dirZ * offset)
            .setCcdEnabled(true)
